...
def sampling(case_path, machine,model_path, case_name,sampling_procs,qname, max_npr_yz):
    ...
    command = "mv" +  '  Macros.' + machine  +  '  Macros.' + machine + '.previous'
    os.system(command)
    ...
    with open('mct.buildlib', 'r', encoding = 'utf-8') as f1, open('mct.buildlib.new', 'w', encoding = 'utf-8') as f2:
        for line in f1:
            ...
            f2.write(line)
    command = 'mv  mct.buildlib  mct.buildlib.previous'
    os.system(command)
    ...
    with open('pio.buildlib', 'r', encoding = 'utf-8') as f1, open('pio.buildlib.new', 'w', encoding = 'utf-8') as f2:
        for line in f1:
            ...
            f2.write(line)
    command = 'mv  pio.buildlib  pio.buildlib.previous'
    os.system(command)
    ...
    with open('Makefile', 'r', encoding = 'utf-8') as f1, open('Makefile.new', 'w', encoding = 'utf-8') as f2:
        for line in f1:
            ...
            f2.write(line)
    command = 'mv  Makefile  Makefile.previous'
    os.system(command)
    ...
    for tmp_procs in sampling_procs:
        ...
        command = "mv  env_mach_pes.xml  env_mach_pes.xml.previous"
        os.system(command)
    ...
    ##############################
        ...
        #num_procs
        if arch == ['SBATCH']:
            num_node = str(int(math.ceil(int(max_procs) / ntaskspernode)))
        ...
        #add intel mpi config
        mpienv = "setenv I_MPI_FABRICS shm:dapl\setenv I_MPI_DAPL_UD 1\setenv I_MPI_DAPL_UD_RDMA_MIXED 1\setenv I_MPI_LARGE_SCALE_THRESHOLD 8192\setenv I_MPI_DAPL_UD_ACK_SEND_POOL_SIZE 8704\setenv I_MPI_DAPL_UD_ACK_RECV_POOL_SIZE 8704\setenv I_MPI_DAPL_UD_RNDV_EP_NUM 2\setenv DAPL_UCM_REP_TIME 8000\setenv DAPL_UCM_RTU_TIME 8000\setenv DAPL_UCM_RETRY 10\setenv DAPL_UCM_CQ_SIZE 2000\setenv DAPL_UCM_QP_SIZE 2000\setenv DAPL_UCM_DREQ_RETRY 4\setenv DAPL_UCM_DREP_TIME 200\setenv DAPL_UCM_WAIT_TIME 10000"
        mpienvs = mpienv.split('\\')
        for item in mpienvs:
            command = 'sed -i "/^setenv LBSUBMIT.*/a\\' + item + '" '+ case_name + '.' +machine+ '.run'
            os.system(command)
        ...
        if licom_result:
            while(not os.path.exists(case_path + "/sampling/result_module/0.csv") and run_count < len(licom_result)):
                ...
                jobfile = open(case_name + "id.log", 'r').read()
                jobid = re.findall(r"\d+", jobfile)[0]
                case_out = os.popen('find . -maxdepth 1  -name "'  + '*'  + jobid + '*'+'"').read().split("\n")[0]
                if case_out != '':
            ...
            while not os.path.exists(case_path + "/sampling/result_module/0.csv"):
                time.sleep(10)
                jobfile = open(case_name + "id.log", 'r').read()
                jobid = re.findall(r"\d+", jobfile)[0]
                case_out = os.popen('find . -maxdepth 1  -name "'  + '*'  + jobid + '*'+'"').read().split("\n")[0]
