...
set mapfilevarname = (map_a2of_file map_a2os_file map_o2af_file map_o2as_file \
...
foreach mapfile ($map_a2of_file $map_a2os_file $map_o2af_file $map_o2as_file \
  ...
  if ($mapfile != idmap) then
    if(-r "$DIN_LOC_ROOT/$mapfile") then
      set $mapfilevarname[$i] = $DIN_LOC_ROOT/$mapfile
      cat >> $CASEBUILD/cpl.input_data_list <<EOF
      $mapfilevarname[$i] = $DIN_LOC_ROOT/$mapfile
EOF
    else if(-r "$DIN_LOC_ROOT/cpl/cpl6/$mapfile") then
      set $mapfilevarname[$i] = $DIN_LOC_ROOT/cpl/cpl6/$mapfile
      cat >> $CASEBUILD/cpl.input_data_list <<EOF
      $mapfilevarname[$i] = $DIN_LOC_ROOT/cpl/cpl6/$mapfile
EOF
         else if(-r "$DIN_LOC_ROOT/data_licom/$mapfile") then
           set $mapfilevarname[$i] = $DIN_LOC_ROOT/data_licom/$mapfile
           cat >> $CASEBUILD/cpl.input_data_list <<EOF
           $mapfilevarname[$i] = $DIN_LOC_ROOT/data_licom/$mapfile
EOF
    endif
  endif
end
